...
{
	char Str[MAX_TEXT_LENGTH + 5]; // 5 for \n etc.
	char *pStr;
	struct ENTRY *pEntry;
	...
	while (fgets (Str, MAX_TEXT_LENGTH + 5, IniFile) != NULL)
	{
		pStr = strchr (Str, '\n');
		if (pStr != NULL)
		{
...
{
	char Sec[MAX_KEY_LENGTH + 2];
	char iSec[MAX_KEY_LENGTH + 2];
	struct ENTRY *pEntry;
...
{
	char Search[MAX_KEY_LENGTH + 2];
	char Found[MAX_KEY_LENGTH + 2];
